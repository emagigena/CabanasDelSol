...
export default function ProductCard({ product }) {
  ...
  return (
    <div style={{ borderRadius: "20px" }} className="border border-grey shadow-lg rounded-lg relative">
      <Link href={`/products/${product.id}`} className="group">
          ...
          {Array.isArray(product.image) && (
            ...
            >
              ...
              <div className="absolute bottom-2 left-2 flex items-center justify-center bg-white rounded-full p-2">
                <AccessibilityIcon />
                <span className="text-gray-500 text-sm ml-1">{product.maxCapacity}</span>
              </div>
        ...
        <div className="p-4 text-sm">
          <h3 className="h3-4 text-sm italic text-gray-400 max-h-16 overflow-hidden">{product.description}</h3>
        </div>
      </Link>
